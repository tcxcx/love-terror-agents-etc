  ...
  const handleSubmit = async (formData: any) => {
    ...
    try {
      ...
      if (chainId === Avalanche.chainId) {
        tokenAddress = AvalancheTokens[0].address;
      } else {
    ...
    <div>
      <Form {...formData}>
        <form
          ...
          className="flex flex-col p-2 md:p-5 w-full mx-auto rounded-md max-w-3xl gap-2 border bg-white"
        >
          <MultiStepViewer
            formData={formData}
            loading={isPeanutLoading}
            onSubmit={onSubmit}
            tokenAmount={tokenAmount}
            setTokenAmount={function (amount: number): void {
