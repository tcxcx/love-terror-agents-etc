...
const MessageLoader = () => (
  <div className="flex items-center space-x-2 text-purple-400/70">
    <div className="w-2 h-2 rounded-full bg-purple-400/70 animate-bounce" style={{ animationDelay: '0ms' }} />
    <div className="w-2 h-2 rounded-full bg-purple-400/70 animate-bounce" style={{ animationDelay: '150ms' }} />
    <div className="w-2 h-2 rounded-full bg-purple-400/70 animate-bounce" style={{ animationDelay: '300ms' }} />
  </div>
);
...
const { messages, input, handleInputChange, handleSubmit, isLoading, stop } = useChat({
  ...
  api: '/api/chat',
  onFinish: async () => {
    await mutate('/api/game/state');
    await mutate(`/api/game/${gameId}`);
  },
});
...
const renderMessage = (message: any, index: number) => {
  ...
  const shouldShowLoader = isLastAIMessage && !showNewMessage;
